...
def Main():
	...
	parser.add_argument("-M", "--contact", type=int, metavar='', help='Add Mobile Number along with Name')
...
#Code to Display the Contact List
	if args.list=="all":
		...
		print("Type all for displaying the Contact List")

#Code to Add Name, Email and Mobile#
	if args.name:
		print ("Name is : %s\nEmail is : %s\nMobile Number is : %s" % (args.name, args.email, args.contact))
		if (len(str(args.contact))!=10):
			...
			"%s"%(args.name):{
                  "Name":"%s"%(args.name),
                  "Contact Number":"%s"%(args.contact),
                  "Email Id":"%s"%(args.email)
	        }
	}
